...
function isValidSophieAddress(address) {
  validateString(address)

  try {
    const {data: addressBuffer} = bech32.decode(address)
    
    if (!sophieAddressTypes.includes(getAddressType(addressBuffer))) {
      return false
    }
  } catch (e) {
    return false
...
function getSophieAddressNetworkId(addressBuffer) {
  validateBuffer(addressBuffer)

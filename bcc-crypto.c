...
int emscripten_verify(const unsigned char *message, size_t message_len, const unsigned char *public_key, const unsigned char *signature){
  return bcc_crypto_ed25519_sign_open(message, message_len, public_key, signature);
}

EMSCRIPTEN_KEEPALIVE
void emscripten_to_public(const unsigned char *secret_key, unsigned char *public_key) {
  bcc_crypto_ed25519_publickey(secret_key, public_key);
}
...
EMSCRIPTEN_KEEPALIVE
void emscripten_bcc_memory_combine(const uint8_t *pass, const uint32_t pass_len, const uint8_t *source, uint8_t *dest, uint32_t sz){
